...
class OfferRepository extends Repository
{
    ...
    {
        $additionalGroups = $offer->additionalGroups()->get();
        foreach($additionalGroups as $additionalGroup) {
            $additionalGroup->date = trim(preg_replace('/\s+/',' ', explode('-', $additionalGroup->name)[1]));
        }
        $additionalGroups = $additionalGroups->sortBy('date');
